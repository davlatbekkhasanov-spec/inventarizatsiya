...
def database_url_candidates() -> list[str]:
    urls: list[str] = []
    for key in ("DATABASE_URL", "DATABASE_PRIVATE_URL", "DATABASE_PUBLIC_URL", "POSTGRES_URL"):
...
def to_sync_env_url(async_url: str) -> str:
    return normalize_database_url(async_url).replace("postgresql+asyncpg://", "postgresql://")
